...
const getScriptUrl = (platform, mode) => {
  if (typeof window === "undefined") return `/api/cli-tools/claude-setup?platform=${platform}&mode=${mode}`;
  const url = new URL("/api/cli-tools/claude-setup", window.location.origin);
...
export default function ClaudeAutoSetupModal({
  isOpen,
  onClose,
  tool,
  baseUrl,
  ...
  const buildInstallCommand = () => {
    ...
    const allEnv = [
      ...
      ["API_KEY", apiKey],
      ["OPUS_MODEL", modelValues.opus],
      ["SONNET_MODEL", modelValues.sonnet],
      ["HAIKU_MODEL", modelValues.haiku],
    ].filter(([, value]) => value);
  ...
  return (
    <>
      <Modal isOpen={isOpen} onClose={onClose} title="Claude Code - Automatic Setup" size="full">
        <div className="flex flex-col gap-5">
          <div className="rounded-lg border border-blue-500/30 bg-blue-500/10 px-3 py-2 text-sm text-text-main">
            Configure Claude Code to use <span className="font-semibold">9Router Proxy</span> with one command. Select your OS and model mapping, then copy the generated terminal command.
          </div>

          <section className="flex flex-col gap-2">
            <div className="flex items-center justify-between gap-3">
              <h3 className="text-sm font-semibold text-text-main">Step 1: Install Claude Code</h3>
              <Button variant="ghost" size="sm" onClick={() => copyCommand(CLAUDE_INSTALL_COMMAND, "claude-install")}>
                <span className="material-symbols-outlined text-[14px] mr-1">{copied === "claude-install" ? "check" : "content_copy"}</span>
                {copied === "claude-install" ? "Copied!" : "Copy"}
              </Button>
            </div>
            <p className="text-xs text-text-muted">If Claude Code is not installed, run this command first:</p>
            <pre className={commandBlockClass}>{CLAUDE_INSTALL_COMMAND}</pre>
          </section>
          ...
          <section className="flex flex-col gap-2 border-t border-border pt-4">
            ...
            <div className="rounded-lg border border-border bg-surface/30 px-3 py-2 text-xs text-text-muted">
              ...
              <ol className="list-decimal pl-4 space-y-1">
                ...
                <li>Paste it into the terminal where Claude Code is installed.</li>
                <li>Restart Claude Code or open a new terminal, then run <code className="px-1 bg-black/5 dark:bg-white/5 rounded">claude</code>.</li>
              </ol>
            </div>
          </section>
          ...
          <section className="flex flex-col gap-2 border-t border-border pt-4">
            <div className="flex items-center justify-between gap-3">
              <h3 className="text-sm font-semibold text-red-600 dark:text-red-400">Uninstall 9Router Proxy config</h3>
            ...
            <p className="text-xs text-text-muted">This removes only the 9Router Proxy Claude Code environment values and preserves other settings.</p>
