...
function AddCompatibleModal({ variant, isOpen, onClose, onCreated }) {
  ...
  const [authScheme, setAuthScheme] = useState("x-api-key");

  // openai: reset baseUrl when apiType changes; anthropic: reset checks + authScheme when opened
  useEffect(() => {
    ...
    try {
      const res = await fetch("/api/provider-nodes", {
        ...
        body: JSON.stringify({
          ...
          ...(config.hasApiType ? { apiType: formData.apiType } : {}),            baseUrl: formData.baseUrl,
            type: config.type,
            ...(variant === "anthropic" ? { authScheme } : {}),
          }),
      });
    ...
    try {
      const res = await fetch("/api/provider-nodes/validate", {
        ...
        body: JSON.stringify({
          ...
          apiKey: checkKey,            type: config.type,
