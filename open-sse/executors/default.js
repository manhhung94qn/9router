  ...
  buildUrl(model, stream, urlIndex = 0, credentials = null) {
    ...
    resolveAuthDescriptor(credentials) {
      if (this.provider?.startsWith?.("anthropic-compatible-")) {
        ...
        // Respect explicit authScheme selector from the frontend
        if (authScheme === "bearer") {
          return { combined: true, header: "Authorization", scheme: "bearer", anthropicVersion: true };
        }
    ...
    buildHeaders(credentials, stream = true) {
      ...
      const desc = AUTH_DESCRIPTORS[this.provider] || this.resolveAuthDescriptor(credentials);
